...
### Task 1.3: Traceroute
[traceroute.py ](https://github.com/HasanFiratKilic/Seedlab/blob/main/Packet-Sniffing-and-Spoofing/traceroute.py)
...
After running the program on the attacker machine, we establish a telnet connection from any victim machine to another. For example, the following code opens a telnet connection from machine 10.9.0.5 to machine 10.9.0.6 with the username 'seed'.

    telnet 10.9.0.6 -l seed

Then, it asks the user to type the password in an interactive prompt.
Meanwhile the program running on the attacker machine [sniffing_password.c]() captures the packets and prints them to the screen.
![psw.png](psw.png)
